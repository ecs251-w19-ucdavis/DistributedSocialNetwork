...
} from './wait-for-mastodon-to-start'
...
const DB_NAME = 'mujo_development'
const DB_USER = 'mujo'
const DB_PASS = 'mujo'
...
async function cloneMastodon() {
  try {
    ...
    await exec(`git fetch origin --tags`, {
      cwd: mastodonDir
    }) // may already be cloned, e.g. in CI
    await exec(`git checkout ${GIT_TAG}`, {
      cwd: mastodonDir
    })
...
async function runMastodon() {
  ...
  const log = fs.createWriteStream('mastodon.log', {
    flags: 'a'
  })
