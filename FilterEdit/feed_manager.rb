...
class FeedManager
  ...
  def filter_from_home?(status, receiver_id)
    ...
    # count=0
    if phrase_filtered?(status, receiver_id, :home)
      # for filtered_id in status.filtered_by
      if !status.filtered_by.include?(receiver_id)
        status.num_of_filtered += 1
        status.filtered_by << receiver_id
      end
      return true
    end
  ...
  def filter_from_mentions?(status, receiver_id)
    ...
    # count=0
    if phrase_filtered?(status, receiver_id, :notifications)
      # for filtered_id in status.filtered_by
      if !status.filtered_by.include?(receiver_id)
        status.num_of_filtered += 1
        status.filtered_by << receiver_id
  ...
  def phrase_filtered?(status, receiver_id, context)
    ...
    return true if status.num_of_filtered >= 2        # 服务器里有3个账号，2个可以代表大多数了
